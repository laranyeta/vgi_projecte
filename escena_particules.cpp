...
// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material.
	...
	case PROVA_PLANETA:
		...
		//particleSystem(sh_programID, MatriuVista, MatriuTG, time, PLANETES[0].getPosition(), ParticleType::Nau)
		// Dibuix geometria Mar
		color_Mar.r = 0.5;	color_Mar.g = 0.4; color_Mar.b = 0.9; color_Mar.a = 0.5;
		// Definició propietats de reflexió (emissió, ambient, difusa, especular) del material pel color de l'objecte.
		SeleccionaColorMaterial(sh_programID, color_Mar, sw_mat); //sw_material = FALSE (GL_FALSE)
...
void processaFisica()
{
	...
	for (int i = 0; i < PLANETES.size(); i++)
	{
		...
		if (i == 0) {
			...
			//particleSystem_Sol.ajustarParticle_Sol(0.16f, PLANETES[i].getPosition());
		}
...
void particleSystem(GLuint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, float time, glm::vec3 posObj, ParticleType pt) {
	static ParticleSystem ps(100, pt, posObj); //cal nombre de particules a generar
